...
class AlgolCheck(token: String, version: String = "v2.7") {
  ...
  def post(id: String, message: String): Unit = {
    val result = Unirest.post(graphURL + id).field("message", message).field("access_token", token).asJson.getBody.getObject
  ...
  def like(id: String): Unit = {
    val result = Unirest.post(graphURL + id + "/likes").field("access_token", token).asJson.getBody.getObject
